...
def unpack_dna(dna):
    ...
    hidden_layers = dna[1]
    hidden_nodes = dna[2]
    return epoch, hidden_layers, hidden_nodes

def make_tf_nn(hidden_layers, hidden_nodes, input_dim, output_dim):
    nn = tf.keras.models.Sequential()
    first_layer = True
    for _ in range(hidden_layers):
        if first_layer:
            nn.add(tf.keras.layers.Dense(hidden_nodes, input_dim=input_dim, activation='sigmoid'))
            first_layer = False
        else:
            nn.add(tf.keras.layers.Dense(hidden_nodes, activation='sigmoid'))
    if first_layer:
...
def make_tf_nn_with_dna(dna, input_dim, output_dim):
    epoch, hidden_layers, hidden_nodes = unpack_dna(dna)
    return make_tf_nn(hidden_layers, hidden_nodes)

def make_nn(hidden_layers, hidden_nodes, input_dim, output_dim):
...
def make_nn_with_dna(dna, input_dim, output_dim):
    epoch, hidden_layers, hidden_nodes = unpack_dna(dna)
    return make_nn(hidden_layers, hidden_nodes, input_dim, output_dim)